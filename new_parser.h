...
StatusType get_line(char *pString){/*returns FALSE the user entered more than 256 characters, TRUE if valid 256 characters, EXIT if exit*/
    ...
    while ( ( c = getchar() ) != EOF){
        if(i == BUFFERSIZE){
            return error_message(line_limit,Edit);
 ...
 * if matches - returns TRUE. else returns FALSE and prints error_message message.*/
    ...
    status = do_order(CmdArray[cmd_index], p_mode, param_num, (char **) paramsArray, *board);/*checks inner validation inside any function. returns TRUE if game is over else FALSE*/
